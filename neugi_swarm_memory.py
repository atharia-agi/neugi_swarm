...
class MemoryManager:
    ...
    def remember(self, memory_type: str, content: str, importance: int = 5, 
    ...
    def consolidate(self) -> int:
        ...
        for content, count in duplicates:
            ...
            c.execute('''DELETE FROM memories WHERE content = ? AND 
                        id NOT IN (SELECT id FROM memories WHERE content = ? 
                        ORDER BY importance DESC LIMIT 1)''', (content, content))
    ...
    def add_message(self, session_id: str, role: str, content: str):
        ...
        msg_id = hashlib.md5(f"{session_id}{content}{datetime.now().isoformat()}".encode()).hexdigest()[:16]
    ...
    def get_conversation(self, session_id: str, limit: int = 20) -> List[Dict]:
        ...
        c = self.conn.cursor()
        c.execute('''SELECT * FROM conversations WHERE session_id = ? 
    ...
    def query_knowledge(self, entity: str = None, relation: str = None) -> List[Dict]:
        ...
        if entity:
            c.execute('''SELECT * FROM knowledge WHERE entity = ? OR target = ?''',
                (entity, entity))
        elif relation:
