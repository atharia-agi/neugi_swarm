...
class AgentManager:
    ...
    def _create_default_agents(self):
        ...
        default_agents = [
            ("aurora", "Aurora", AgentRole.RESEARCHER, 
             ["search", "fetch", "analyze"], ["web_search", "web_fetch", "llm_think"]),
            ("cipher", "Cipher", AgentRole.CODER,
             ["code", "debug", "build"], ["code_execute", "file_write", "llm_think"]),
            ("nova", "Nova", AgentRole.CREATOR,
             ["create", "design", "generate"], ["image_generate", "llm_think", "file_write"]),
            ("pulse", "Pulse", AgentRole.ANALYST,
             ["analyze", "visualize", "report"], ["json_parse", "csv_analyze", "llm_think"]),
            ("quark", "Quark", AgentRole.STRATEGIST,
             ["plan", "decide", "optimize"], ["llm_think"]),
            ("shield", "Shield", AgentRole.SECURITY,
             ["scan", "audit", "protect"], ["code_debug", "web_fetch"]),
            ("spark", "Spark", AgentRole.SOCIAL,
             ["post", "engage", "schedule"], ["send_telegram", "send_discord"]),
            ("ink", "Ink", AgentRole.WRITER,
             ["write", "edit", "proofread"], ["file_write", "llm_think"]),
            ("nexus", "Nexus", AgentRole.MANAGER,
    ...
    def _save_agent(self, agent: Agent):
        ...
        c.execute('''INSERT OR REPLACE INTO agents VALUES (?,?,?,?,?,?,?,?,?,?,?,?)''',
            (agent.id, agent.name, agent.role.value, agent.status.value,
             agent.xp, agent.level, agent.tasks_completed,
             json.dumps(agent.capabilities), json.dumps(agent.tools),
             agent.current_task, agent.created_at))
        self.conn.commit()
