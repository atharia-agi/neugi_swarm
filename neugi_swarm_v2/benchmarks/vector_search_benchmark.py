...
def create_test_knowledge_base(num_docs: int = 100) -> list[tuple[str, str]]:
...
def benchmark_indexing_performance() -> dict | None:
...
def benchmark_search_performance() -> dict | None:
...
def benchmark_hybrid_vs_keyword_only() -> dict | None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for query in test_queries:
            ...
            start_time = time.time()
            try:
                search_knowledge(str(index_path), query, limit=10, use_vectors=True)
                end_time = time.time()
            ...
            start_time = time.time()
            try:
                search_knowledge(str(index_path), query, limit=10, use_vectors=False)
                end_time = time.time()
...
def run_benchmarks() -> dict[str, Any] | None:
