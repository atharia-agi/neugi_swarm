...
if TYPE_CHECKING:
    from neugi_swarm_v2.session import SessionConfig
...
try:
    ...
    SecretNotFoundError = None  # type: ignore
    SecretDecryptionError = None  # type: ignore
...
def load_config(
    ...
    **overrides,
    ...
    API keys are retrieved exclusively through SecretManager — never from
    plaintext config.json. If a plaintext key is found in config.json on
    first load, it is migrated to SecretManager and removed from the file.
    ...
    Raises:
        ConfigurationError: If SecretManager is unavailable or fails to
            retrieve a stored secret (no silent fallback).
    """
    config = NeugiConfig()
    ...
    if config_path and Path(config_path).exists():
        try:
            ...
            # Remove api_key from data before building config — never load
            # plaintext API keys from config.json into the config object.
    ...
    # Retrieve API key from SecretManager (encrypted storage)
    config.llm.api_key = _retrieve_api_key_from_secrets(config.neugi_dir)
...
def _retrieve_api_key_from_secrets(neugi_dir: Path) -> str:
    """Retrieve the LLM API key from SecretManager.

    If SecretManager is not available or the secrets database does not exist
    (fresh install with no key stored yet), returns an empty string. If the
    database exists but retrieval fails, raises ConfigurationError.

    Args:
        neugi_dir: Root NEUGI directory containing secrets.db.

    Returns:
        The decrypted API key, or empty string if no key is stored yet.

    Raises:
        ConfigurationError: If SecretManager is unavailable when secrets.db
            exists, or if decryption/retrieval fails.
    """
    secrets_db = neugi_dir / "secrets.db"

    # Fresh install — no secrets database yet. The wizard will store the key later.
    if not secrets_db.exists():
        return ""

    if SecretManager is None:
        raise ConfigurationError(
            "SecretManager is not available but secrets.db exists at "
            f"{secrets_db}. Cannot retrieve API key without SecretManager."
        )

    try:
        manager = SecretManager(db_path=str(secrets_db))
        return manager.get("llm_api_key")
    except SecretNotFoundError:
        # Key not stored yet — fresh install, wizard will store it later.
        return ""
    except SecretDecryptionError as e:
        raise ConfigurationError(
            f"Failed to decrypt API key from SecretManager: {e}"
        ) from e
    except Exception as e:
        raise ConfigurationError(
            f"SecretManager failed to retrieve API key: {e}"
        ) from e
