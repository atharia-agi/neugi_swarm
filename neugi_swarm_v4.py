...
class NeugiMemory:
    ...
    def remember(
        self, agent_id: str, event_type: str, content: str, importance: int = 5
    ):
        conn = sqlite3.connect(self.db_path)
        c = conn.cursor()
        c.execute(
    ...
    def recall(self, query: str, limit: int = 5) -> List[Dict]:
        ...
        c = conn.cursor()
        c.execute(
            "SELECT * FROM episodic WHERE content LIKE ? ORDER BY importance DESC LIMIT ?",
            (f"%{query}%", limit),
        )
        results = c.fetchall()
        conn.close()
...
class SocialClients:
    ...
    def __init__(self):
        # Moltbook
        self.moltbook_key = os.environ.get("MOLTBOOK_KEY")
        self.mb_headers = {
        ...
        # Twitter (GetLate)
        self.twitter_key = os.environ.get("TWITTER_KEY")
        self.tw_headers = {
            "Authorization": f"Bearer {self.twitter_key}",
            "Content-Type": "application/json",
        }
...
class NeugiV4:
    ...
    def __init__(self):
        ...
        self.agents = {
            "twitter": Agent(
                "twitter", "Atharia-Twitter", "social", ["post", "engage"]
            ),
            "moltbook": Agent(
                "moltbook", "Atharia-Moltbook", "social", ["post", "connect"]
            ),
            "research": Agent(
                "research", "Atharia-Research", "research", ["search", "analyze"]
            ),
            "trading": Agent(
                "trading", "Atharia-Trading", "finance", ["analyze", "signal"]
            ),
            "content": Agent(
                "content", "Atharia-Content", "content", ["write", "edit"]
            ),
            # NEW: Auroria agent
            "auroria": Agent(
                "auroria",
