...
class SSHManager:
    ...
    def connect(self, conn_id: str) -> Dict:
        ...
        return {"success": True, "command": " ".join(cmd), "message": "Connection ended"}
    ...
    def copy_file(
        self, conn_id: str, local_path: str, remote_path: str, direction: str = "upload"
    ) -> Dict:
        """Copy files via SCP"""
        conn = self.get_connection(conn_id)
        if not conn:
...
class SSHKeyManager:
    ...
    def generate_key(self, name: str, key_type: str = "rsa", bits: int = 4096) -> Dict:
        ...
        try:
            ...
            return {"success": True, "private_key": key_path, "public_key": f"{key_path}.pub"}
    ...
    def list_keys(self) -> List[Dict]:
        ...
        for f in os.listdir(self.keys_dir):
            if f.endswith(".pub"):
                ...
                keys.append({"name": name, "public_key": content, "path": key_path})
        return keys
...
def main():
    ...
    parser.add_argument(
        "--add", nargs=4, metavar=("NAME", "HOST", "USER", "PORT"), help="Add connection"
    )
    ...
    if args.list:
        ...
        print(
            "Usage: python -m neugi_ssh [--list|--add NAME HOST USER PORT|--connect ID|--exec ID CMD|--keys|--gen-key NAME]"
        )
